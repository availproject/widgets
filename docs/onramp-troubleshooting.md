...
- Chain IDs from middleware `transaction.chainId` and Meld `transaction.cryptoDetails.chainId` are normalized before destination checks: `10`, `"10"`, `"0xa"`, and `"EVM_10"` all identify Optimism. A malformed supplied ID rejects the response instead of bypassing the chain check; a valid different chain still blocks the deposit.
- Returned `transaction.destinationAmount`, `sourceAmount`, and `sourceCurrencyCode` override the checkout quote. A returned transaction payment method or provider takes precedence over session defaults. Partial responses retain previously reported values; missing methods do not overwrite the selected method. The original quote is retained for display when the provider omits a field. Deposit execution always requires a received amount from the settled transaction.
- Transaction history includes local **Onramp** entries with provider, token amount, fiat amount/currency, available payment method, and payment status, including pending and failed sessions. Quoted amounts are identified when returned amounts are unavailable. Entries are saved by session ID as soon as the session is created, and scoped to the wallet and middleware URL. Saved data excludes checkout URLs and unrecognized provider fields.
- Saved entries load when the widget mounts. Opening history refreshes pending sessions immediately, then every five seconds after each completed pass. Focus, online and visibility events refresh sooner. Hidden tabs skip requests, closing history aborts pending reads, and terminal payments stop polling. Refresh failures preserve the last known status. An onramp marked **Completed** means the purchase settled; history polling never executes a deposit or signs a transaction. The history button is available on the deposit funding-method screen.
...
The regression suite uses mocked Meld/proxy responses and wallet/RPC calls. It does not charge a card, sign a transaction or test provider webhook delivery. For the RC payment test, keep the page open, approve the wallet prompts, and verify the deposit explorer receipt as well as the provider's onramp receipt. Pending purchase status resumes from local history after reload; deposit execution does not resume automatically. A retained session ID can be inspected with the existing `window.setRampSessionId(id)` helper, but always check already-submitted transactions before attempting deposit recovery after a reload.
