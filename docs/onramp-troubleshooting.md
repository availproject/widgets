...
- `SETTLED` means the purchase is complete. It triggers the deposit; it no longer means the whole flow succeeded. The actual received amount comes from the transaction, not the earlier quote. The wallet balance is checked immediately, then retried every second up to 60 times before showing a retryable insufficient-balance error (RPC response time can extend the wait). Missing amounts and missing wallets also surface a deposit error with a retry action.
