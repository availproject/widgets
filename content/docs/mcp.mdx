...
For now, install Avail Widgets from the public GitHub registry. No
`components.json` namespace setup is required for `availproject/widgets/nexus`.
The `@avail-widgets/nexus` namespace will be documented after it is listed in
shadcn.
...
`components.json`. Avail Widgets does not require this while it is installed
from the public GitHub registry path.
