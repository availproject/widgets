...
| `config.enableOnRamp`                                                              | Optional | Boolean (defaults to `false`). When enabled, displays the "Pay with Local Currency" funding option.                                         |
| `config.dapps`                                                                     | Optional | Array of external dApp funding options (`{ title, logo, description, targetUrl }[]`). Selecting an option redirects to its URL in a new tab.  |
