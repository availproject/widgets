...
<Callout type="info">
  **Get the exact config for your protocol** using the [Avail Configurator](https://configurator.availproject.org/). It generates the complete `depositConfig` object for deposit and bridge modes based on your contract and token setup.
</Callout>
...
| `config.theme` | Optional | `"dark"`, `"light"`, or `"system"`. Defaults to light; system follows OS changes. Overrides `appearance.mode`. |
