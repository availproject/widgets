...
test("one attempt survives requotes; each usable quote and execution is timed once", async () => {
  ...
  try {
    ...
    assert.equal(new Set(h.records.map(r => r.properties.attempt_id)).size, 1);
...
test("new user flow gets a new attempt while a late result retains its original attempt", async () => {
  ...
  try {
    ...
    const firstId = h.records[0].properties.attempt_id;
    h.telemetry.resetAttempt();
    await h.telemetry.observeSwap("swapWithExactOut", 2, async () => ({}));
    const secondId = h.records.find(r => r.event === "widget_attempt_started" && r.properties.attempt_id !== firstId)?.properties.attempt_id;
    assert.ok(secondId);
    pending.resolve({});
    await operation;
    assert.equal(h.records.at(-1)?.properties.attempt_id, firstId);
    assert.ok(h.records.every(r => !('previous_attempt_id' in r.properties)));
  } finally { h.telemetry.dispose(); }
});
...
test("initialization and balance results before a quote have no invented attempt", async () => {
  ...
  try {
    ...
    assert.ok(h.records.every(r => !('attempt_id' in r.properties)));
  } finally { h.telemetry.dispose(); }
});
...
test("a setup call started before a quote cannot borrow its later attempt ID", async () => {
  ...
  try {
    ...
    setup.resolve();
    await operation;
    assert.ok(h.records[0].properties.attempt_id);
    assert.equal(h.records[1].properties.attempt_id, undefined);
  } finally { h.telemetry.dispose(); }
});
...
for (const mode of ["swap", "send", "deposit"] as const) {
  test(`${mode}: ERC20 commitment uses evidence once and is labelled browser-observed`, async () => {
    ...
    try {
      ...
      assert.equal(commits[0].properties.attempt_id, h.records[0].properties.attempt_id);
...
for (const choice of ["stopped", "rejected"] as const) {
  test(`explicit pre-execution ${choice} is deduplicated and links the next attempt`, async () => {
    ...
    try {
      h.telemetry.beginAttempt("swapWithExactIn");
      const first = h.records[0].properties.attempt_id;
      ...
      assert.notEqual(next.properties.attempt_id, first);
      assert.equal(next.properties.previous_attempt_id, first);
      await h.telemetry.flush();
      const events = h.requests[0].body.batch;
      const logs = h.requests[1].body.resourceLogs[0].scopeLogs[0].logRecords;
      assert.deepEqual(events.map((e: any) => e.uuid), logs.map((l: any) => l.attributes.find((a: any) => a.key === "eventId").value.stringValue));
...
test("backend failures require final-result scope and correct attempt; amounts remain opt-in", () => {
  ...
  try {
    h.telemetry.beginAttempt("swapAndExecute");
    const evidence = { kind: "outcome", attempt_id: h.records[0].properties.attempt_id, eventId: "10000000-0000-4000-8000-000000000003", occurredAt: new Date(1000).toISOString(), authority: "middleware", scope: "final_requested_result", outcome: "failed", committed: true, intentHash: publicIntentHash, estimatedValueUsd: 30 };
...
test("opt-out closes the publisher adapter and suppresses late backend callbacks", () => {
  ...
  try {
    h.telemetry.beginAttempt("swapAndExecute");
    const id = h.records[0].properties.attempt_id;
    h.telemetry.configure({ ...h.settings, config: { ...h.settings.config, disableLogging: true } });
...
test("a backend outcome can arrive before its commitment record without losing either", () => {
  ...
  try {
    h.telemetry.beginAttempt("swapAndExecute");
    const base = { attempt_id: h.records[0].properties.attempt_id, occurredAt: new Date(1000).toISOString(), authority: "middleware", intentHash: publicIntentHash };
...
test("late backend outcome stays joined to its original attempt after a form reset", () => {
  ...
  try {
    ...
    assert.equal(outcome.properties.attempt_id, first.context.attempt_id);
    assert.notEqual(outcome.properties.attempt_id, subscriptions[1].context.attempt_id);
...
test("completed outcome values remain absent without amount opt-in", () => {
  ...
  try {
    ...
    report({ kind: "outcome", attempt_id: h.records[0].properties.attempt_id, eventId: "10000000-0000-4000-8000-000000000008", occurredAt: new Date(1000).toISOString(), authority: "protocol", scope: "final_requested_result", outcome: "completed", committed: true, transactionHash: publicTxHash, estimatedValueUsd: 100, signature: publicTxHash, address });
